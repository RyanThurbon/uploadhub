...
import { useOnWindowResize } from "@/lib/hooks/on-window-resize";
...
const ChartTooltipRow = ({ value, name, color }: ChartTooltipRowProps) => (
  <div className="flex items-center justify-between space-x-8">
    <div className="flex items-center space-x-2">
      <span aria-hidden="true" className={cn("size-2 shrink-0 rounded-sm", color)} />
      <p
        className={cn(
          // commmon
        ...
        <div className={cn("space-y-1 px-4 py-2")}>
          {filteredPayload.map(({ value, name }: { value: number; name: string }, index: number) => (
            <ChartTooltipRow
              ...
              name={name}
              color={getColorClassName(categoryColors.get(name) as AvailableChartColorsKeys, "bg")}
            />
          ))}
        </div>
      </div>
